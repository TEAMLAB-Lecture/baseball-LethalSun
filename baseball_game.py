...
def main():
    ...
    while(True):
        ...
        sb_list = get_strikes_or_ball(user_input,random_number)

        print(f"Strikes : {sb_list[0]} , Balls : {sb_list[1]}")
        ...
        while(True):
            ...
            if(is_yes(con_input) == True):
                is_continue = True
            elif(is_no(con_input) == True):
                is_continue = False
            break
        if(is_continue):
            random_number = str(get_not_duplicated_three_digit_number())
